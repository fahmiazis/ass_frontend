...
class Asset extends Component {
    ...
    downloadData = () => {
        ...
        workbook.xlsx.writeBuffer().then(function(buffer) {
            fs.saveAs(
              ...
              `Data Asset Opname ${moment().format('DD MMMM YYYY')}.xlsx`
            );
          });
    }
    ...
    addDokumen = async (values) => {
        ...
        const {isAdd} = this.props.asset_stock
        if (isAdd) {
    ...
    DownloadMaster = () => {
        const {link} = this.props.asset_stock
        axios({
            url: `${link}`,
            method: 'GET',
    ...
    componentDidUpdate() {
        const {isError, isUpload, isExport, isSync} = this.props.asset_stock
        if (isError) {
            this.props.resetError()
            this.showAlert()
        } 
    ...
    render() {
        ...
        const {dataAsset, isGet, alertM, alertMsg, alertUpload, page} = this.props.asset_stock
            ...
            <>
                <div className={styleTrans.app}>
                    ...
                    <div className={`${styleTrans.mainContent} ${this.state.sidebarOpen ? styleTrans.collapsedContent : ''}`}>
                        <h2 className={styleTrans.pageTitle}>Asset Stock Opname</h2>
                ...
                <Modal isOpen={this.props.asset_stock.isLoading || this.state.isLoading ? true: false} size="sm">
                        <ModalBody>
                        <div>
...
const mapDispatchToProps = {
    ...
    uploadMasterAsset: asset_stock.uploadMasterAsset,
    nextPage: asset_stock.nextPage,
    syncAsset: asset_stock.syncAsset
}
